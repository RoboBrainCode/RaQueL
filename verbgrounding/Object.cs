...
namespace ProjectCompton
{
    class Object
    {
        ...
        public void append(String state, String newValue, String defaultValue)
        {
            /* Function Description : Adds the newValue to the state value 
             * if the state does not exist then it does nothing else if
             * the state value is default then initialize to newValue else
             * append */

            String value = this.getStateValue(state);
            if (value.Equals(defaultValue))
            {
                this.changeState(state, newValue);
            }
            else
            {
                ...
                {
                    if (values[iter].Equals(newValue))
                    {
                        exists = true;
                        break;
                    }
                ...
                if(!exists)
                    this.changeState(state, value + "+" + newValue);
            }
        }

        public void toggle(String state, String val1, String val2)
        {
            /* Function Description : If the state exists the toggle between 
             * the two valuel. */

            String value = this.getStateValue(state);
            if (value.Equals(val1))
            {
                this.changeState(state, val2);
            }
            else if (value.Equals(val2))
            {
                this.changeState(state, val1);
            }
        }

		public double groundingScore(String reference)
		{
			/* Function Description: Computes a score of how much the string reference
			 * refers to this object. In future, object will be an unlabelled segment
			 * with classification deferred till inference and then this function should
             * called an open-vocabulary probability computation like Guadarrama et al RSS 14.
             * However for now, I resort to the following approach - 
             *       references may use class labels, their state values, synonyms etc.
             * M1. number of words in reference contained in object-name
             *     eg: bag -> garbageBag  with score 1, bin -> garbageBin with score 1, etc.
             *         failure case- faucet will not map to sink, container wont map to kettle etc.
             * M2. Semantic-Distance on word-net
             *     eg: will match faucet -> sink, container -> kettle etc.
             *     M2 algorithm takes reference and class description of the object-name
             *        object-name class is encoded as nameInCamelCase_uniqueID
			 */

			double score = 0;
			String objWords = Global.base_ (this.uniqueName);
			String name = "";
			for (int i=0; i<objWords.Length; i++) 
			{
				if (objWords [i] >= 'a' && objWords [i] <= 'z')
					name = name + objWords [i];
				else if (objWords [i] >= 'A' && objWords [i] <= 'Z')
					name = name + " ";
			}
			SentenceSimilarity sm = new SentenceSimilarity();
			score = sm.GetScore (name, reference);
			return score;
		}
