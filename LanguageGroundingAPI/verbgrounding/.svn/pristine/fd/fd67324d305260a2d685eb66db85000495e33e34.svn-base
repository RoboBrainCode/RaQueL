...
namespace ProjectCompton
{
    class NoiseRemoval
    {
		...
		public static void readNoiseFreeTestDataFromFile(List<int> test, Parser prs, Logger lg)
		{
			...
			int index = 0;
			//read the xml file
			XmlTextReader reader = new XmlTextReader(Constants.rootPath+"test_instruction.xml");
			int start = 0, end = 0;
						...
						if(reader.Name.Equals("point"))
					   	{
							...
							Console.WriteLine ("Entering " + String.Join (", ", parsedSentence [test [index]].Item3.Select (x => x.getName ())));
							alignment_ = null;
							insts_ = null;
							index++;
							if (index == 12) //I have been lazy enough to remove noise only till index 25 :( - Feb 20, 2015
								return;
						}
					break;
				}
