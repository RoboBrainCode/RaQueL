...
namespace ProjectCompton
{
    class VeilTemplate
    {
        ...
        public VeilTemplate(Clause cls, List<Instruction> inst, Environment env, int entryIndex, Simulator sml)
        {
			...
			this.predicatesPost = VeilTemplate.takeOnlyPositivePredicates(this.generalizePredicateList(this.predicatesPostOld,this.zVariablePredicatePost,this.xiOrigMappingPredicatePost));
			#endregion
        }

		public VeilTemplate(List<String> constraint, Clause cls, Environment env, int entryIndex, Simulator sml)
		{
			//Function Description: Directly adding veil template
			this.cls = cls;
			this.env = env;
