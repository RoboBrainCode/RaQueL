...
namespace ProjectCompton
{
    class Learning// : Tester
    {
		                                                          ...
		                                                          Dictionary<String, Double> weights)
		{
			...
			features ["argprior"] = splitter.Aggregate(0.0,(sum,cstr_) => sum + tester.inf.ftr.getPredicateFreq(cstr_))/(splitter.Length*tester.inf.ftr.zPredFreq + Constants.epsilon);
			...
			double totalScore = Math.Exp(weights["w_le"]*features["le"] + weights["w_lerecall"]*features["lerecall"] +
			                             weights["w_dscp"]*features["dscp"] + weights["w_prior"]*features["prior"] + 
			                             weights["w_argprior"]*features["argprior"] + weights["w_rel"]*features["rel"] + 
			                             weights["w_end"]*features["end"] + weights["w_bias"]*features["bias"] + 
		                                    ...
		                                    List<Instruction> instPrev, Dictionary<String, Double> weights, List<object> param, System.IO.StreamWriter sw)
		{
			...
			foreach (LexicalEntry vt in vtList)
			{
				...
				features["argprior"] = cstrSplit.Aggregate(0.0,(sum,cstr_) => sum + tester.inf.ftr.getPredicateFreq(cstr_))/(cstrSplit.Length*tester.inf.ftr.zPredFreq + Constants.epsilon);
				...
				double totalScore = Math.Exp(mapCost + weights["w_lerecall"]*features["lerecall"] + weights["w_dscp"]*features["dscp"] 
				                             + weights["w_sensim"]*features["sensim"]+ weights["w_prior"]*features["prior"]
				                             + weights["w_argprior"]*features["argprior"] + weights["w_rel"]*features["rel"] 
				                             + weights["w_end"]*features["end"]+ weights["w_trans"]*features["trans"]+ weights["w_argtrans"]*features["argtrans"]);
		...
		public List<double[]> analyticGradientDescent(List<int> train, int evalMetric, int fold)
		{
			...
			for (int mth = 0; mth < this.testObj.methods.Count(); mth++) 
			{
				...
				double [] weight = //new double[15]{0.00126810957920883,9.09443427657785E-05,0,0,0.00511791544160663,0.000385611813672254,0,0,0.00740168217600723,0.00133940039043196,0.000250469975184572,0.0083253532592022,0,0,0};
