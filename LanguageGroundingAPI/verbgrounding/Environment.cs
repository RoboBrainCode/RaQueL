...
namespace ProjectCompton
{
    ...
    class Environment
    {
        ...
        public bool isRelative(Object obj1, String obj2, String relativePos)
        {
            /* Function Description : Returns true if relativePos(obj,obj2) holds
             * and false otherwise */
            int i = -1, j = -1;
            for (int iter = 0; iter < objects.Count(); iter++)
            {
                if (obj1.uniqueName.Equals(objects[iter].uniqueName, StringComparison.OrdinalIgnoreCase))
                    i = iter;
                if (obj2.Equals(objects[iter].uniqueName, StringComparison.OrdinalIgnoreCase))
                    j = iter; 
            }
            ...
            if (i != -1 && j != -1)
            {
                foreach (Tuple<int, int, string> tmp in this.relativePosition)
                {
                    if (tmp.Item1 == i && tmp.Item2 == j && tmp.Item3.Equals(relativePos, StringComparison.OrdinalIgnoreCase))
                        return true;
                }
            }
            return false;
        ...
        {
            /* Function Description : Remove all pointers of this object
             * from the relative position.*/
            int i = -1;
            for (int iter = 0; iter < this.objects.Count(); iter++)
            {
                if (this.objects[iter].uniqueName.Equals(obj.uniqueName,StringComparison.OrdinalIgnoreCase))
                    i = iter;
            }

            if (i != -1)
            {
                List<Tuple<int, int, string>> newRelativePosition = new List<Tuple<int, int, string>>();
                for (int iter = 0; iter < relativePosition.Count(); iter++)
                {
                    if (this.relativePosition[iter].Item1 != i && this.relativePosition[iter].Item2 != i
                        || this.relativePosition[iter].Item3.Equals("inside",StringComparison.OrdinalIgnoreCase))
                        newRelativePosition.Add(this.relativePosition[iter]);
                }
			...
			for (int i=0; i<this.objects.Count(); i++) 
			{
				...
				for (int j=0; j<this.objects.Count(); j++) 
				{
					if (!this.objects [j].affordances_.Contains ("IsPlaceableOn") && 
						...
						//objects are being declared to be on sink or in microwave when in fact they are not.
						continue;
